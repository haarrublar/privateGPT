...
def main():
    ...
    db = Chroma(
        ...
        collection_metadata={"hnsw:space": "cosine"}  # Optional optimization
    )

    retriever = db.as_retriever(
        search_kwargs={
            "k": target_source_chunks,  # Increase from 4 to 6-8 for more context
        }
    )
    ...
    llm = OllamaLLM(
        model=model,
        callbacks=callbacks,
        num_ctx=4096,  # Increase context window if supported
    )
